...
namespace emmVRC.Menus
{
    public class UserTweaksMenu
    {
        ...
        public static void Initialize()
        {
            ...
            PlayerNotesButton = new QMSingleButton(UserTweaks, 1, 0, "Player\nNotes", () => { PlayerNotes.LoadNote(QuickMenuUtils.GetQuickMenuInstance().field_APIUser_0.id, QuickMenuUtils.GetQuickMenuInstance().field_APIUser_0.displayName); }, "Allows you to write a note for the specified user");
            TeleportButton = new QMSingleButton(UserTweaks, 2, 0, "Teleport\nto player", () => { if (Configuration.JSONConfig.RiskyFunctionsEnabled) VRCPlayer.field_VRCPlayer_0.transform.position = QuickMenuUtils.GetQuickMenuInstance().field_VRCPlayer_0.transform.position; }, "Teleports to the selected player. Requires risky functions");
        }
